...
def set_location_bounds(map_bounds_in):
    ...
    map_bounds = map_bounds.replace('(','').replace(')','').replace(' ', '').split(',')
...
def generate_client_macs(num_clients, num_aps):
    ...
    for client in range(num_clients):
        ...
        for mac_part in range(6):
            ...
            client_mac += ''.join(random.choice('0123456789abcdef') for i in range(2))

            if mac_part < 5:
                client_mac += ':'
            else:
                client_macs.append({'client_mac':client_mac,
...
def generate_ap_macs(num_aps, num_clients):
    ...
    for ap in range(num_aps):
        ap_mac = ''
        for mac_part in range(6):
            ap_mac += ''.join(random.choice('0123456789abcdef')
            for i in range(2))
            if mac_part < 5:
                ap_mac += ':'
            else:
                ap_macs.append({"ap_mac":ap_mac,
...
def generate_location_data():
    ...
    device_list = [{'os':'Android','manufacturer':'Samsung'},
    ...
    epoch = \
    (date_time_now - datetime.datetime.utcfromtimestamp(0)).total_seconds() \
    * 1000.0
    ...
    for ap_index, ap_mac in enumerate(ap_macs):
        ...
        for client_index, client_mac in enumerate(client_macs):
            ...
            ipv4 = None
            ssid = None
            if client_mac["associated"] == 1 and \
            client_mac["ap_associated"] == ap_index:
                ipv4 = "192.168.0." + str(client_index)
                ssid = "SimulatorWifi"

            observations.append({'clientMac': client_mac["client_mac"],
                'ipv4': ipv4,
                'ipv6': None,
                'location': {
                    'lat': \
                    random.uniform(float(map_bounds[0]),float(map_bounds[2])),
                    'lng': \
                    random.uniform(float(map_bounds[1]),float(map_bounds[3])),
                ...
                'seenEpoch': epoch,
                'seenTime': date_time_now.isoformat(sep='T', timespec='auto'),
                'ssid': ssid})

        ap_data.append({'data': {'apFloors': [],
                           'apMac': ap_mac["ap_mac"],
                           'apTags': [],
...
def update_location_data(ap):
    ...
    date_time_now = datetime.datetime.now()
    epoch = (date_time_now - datetime.datetime.utcfromtimestamp(0)).total_seconds() * 1000.0
    ...
    for observation in observations:
        observation["location"]["lat"] = random.uniform(float(map_bounds[0]),float(map_bounds[2]))
        observation["location"]["lng"] = random.uniform(float(map_bounds[1]),float(map_bounds[3]))
        observation["location"]["unc"] = random.uniform(0,10)
        observation["rssi"] = random.randint(25,120)
        observation["seenEpoch"] = epoch
        observation["seenTime"] = date_time_now.isoformat(sep='T', timespec='auto')
